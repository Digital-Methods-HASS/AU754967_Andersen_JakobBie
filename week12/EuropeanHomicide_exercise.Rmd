...
```{r plotting homicides per 100k, eval = TRUE}
...
```{r faceting the graph, eval = TRUE}
unique(Western_Europe$Country)
...
```{r Plotting the Danish Monarchs, eval = TRUE}
kings <- read_csv2("data/danish_monarchs_rettet.csv")

kings %>% 
  mutate(Midyear = End_year_reign - Duration_of_reign/2) %>% 
  ggplot(aes(x = Midyear, y = Duration_of_reign)) +
  geom_smooth(color = "red") +
  geom_point(color = "blue") +
  labs(title = "Danish Monarchs and their Duration of Reign through Time", x = "Year", y = "Duration of Reign")

#Characterize the trend: The datasets don't really seem to correlate, but there is a little tendency towards falling homicide rates and increasing durations of reign.
```
...
The general consensus that can be reached from comparing to two datasets, is the fact that in our daily lives we tend to encounter less homicides and we also tend to live longer than before. This could very well be used in an argument to prove how humanity has become more civilized over time. However, the datasets only focus on a select few countries in Europe and the monarchs dataset is only about danish monarchs. Therefore, the researchgroup is not broad enough to deliver a general statement on humanity as a whole.